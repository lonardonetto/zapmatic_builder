    ...
    public function duplicate(){
        try {
            ...
            if(empty($ids)){
                ms([
                    "status" => "error",
                    "message" => __('Please select an item to duplicate')
                ]);
            }

            // Busca o template original
            $template = db_get("*", TB_WHATSAPP_TEMPLATE, [
                "ids" => $ids,
                "team_id" => $team_id,
                "type" => 1
            ]);
            ...
            $insert = db_insert(TB_WHATSAPP_TEMPLATE, $new_data);
            ...
            if (function_exists('delete_option_cache')) {
                delete_option_cache();
            }

            // Retorna sucesso com flag para recarregar
            ms([
                "status" => "success",
