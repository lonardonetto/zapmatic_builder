...
class Plugins extends \CodeIgniter\Controller
{
    ...
    public function do_install_mod()
    {
        $end_point_mod =  'https://test00.nd-host.com/';
        $purchase_code = urlencode(trim(post("purchase_code")));
        $website = base_url();

        $params = [
            "domain" => urlencode($website),
            "purchase_code" => $purchase_code,
            "is_main" => 0,
            "module" => 1,
            "mod" => $purchase_code
        ...
        $result = @file_get_contents($end_point_mod . "install_nw.php?" . http_build_query($params), false, stream_context_create($this->stream_opts));
        $result = json_decode($result);
    ...
    public function system_updater_progress_page($update_id = 0)
    {
        $updater = new \Core\Plugins\Controllers\System_updater();
        return $updater->progress_page($update_id);
    }
