...
class Whatsapp_contact extends \CodeIgniter\Controller
{
    ...
    public function delete($ids = "")
    {
        ...
        // Se vem do POST, usar os dados do POST
        if (empty($ids) && post('ids')) {
            $ids = post('ids');
        }
    ...
    public function do_import_contact($ids = "")
    {
        ...
        if (!empty($_FILES) && is_array($_FILES['files']['name'])) {
            ...
            if (!$check_size) {
                ms([
                    "status" => "error",
                    "message" => __(sprintf("Unable to upload a file larger than %sMB", $max_size/1024))
                ]);
            }
        ...
        if (count($data_semicolon) > count($data_comma)) {
            $delimiter = ';';
        } else {
            $delimiter = ',';
        }
    ...
    public function delete_invalid($ids = false)
    {
        $team_id = get_team("id");
        $pid = db_get("*", TB_WHATSAPP_CONTACTS, ["ids" => $ids, "team_id" => $team_id]);
        ...
        db_delete(TB_WHATSAPP_PHONE_NUMBERS, ['pid' => $pid->id, "is_valid" => 2]);
    ...
    public function delete_duplicate($ids = false)
    {
        $team_id = get_team("id");
        $pid = db_get("*", TB_WHATSAPP_CONTACTS, ["ids" => $ids, "team_id" => $team_id]);
        ...
        foreach ($results as $record) {
            if (isset($seenPhones[$record->phone])) {
    ...
    public function exportar_csv($ids = false, $delimiter_type = 'comma')
    {
        $delimiter = ($delimiter_type == 'semicolon') ? ';' : ',';
        $team_id = get_team("id");
        $db = \Config\Database::connect();
        $builder = $db->table(TB_WHATSAPP_PHONE_NUMBERS);
        $builder->select('phone, params');
        ...
        $contact_group = db_get("*", TB_WHATSAPP_CONTACTS, ["ids" => $ids, "team_id" => $team_id]);
        ...
        $builder->where('pid', $contact_group->id);
        $filename = "export_" . str_replace(" ", "_", $contact_group->name) . "_" . date('Ymd') . ".csv";
        ...
        if (!empty($results)) {
            foreach ($results as $row) {
                $params = $this->decode_json($row->params);
                if (is_array($params)) {
                    $all_param_keys = array_merge($all_param_keys, array_keys($params));
                }
            }
        }
        $all_param_keys = array_unique($all_param_keys);
        $header = array_merge(['Phone'], $all_param_keys);
        ...
        if (!empty($results)) {
            foreach ($results as $row) {
                ...
                $csv_line = [$row->phone];
                foreach ($all_param_keys as $key) {
                    $csv_line[] = isset($params[$key]) ? $params[$key] : '';
                }
                fputcsv($output, $csv_line, $delimiter);
            }
        }
    ...
    private function decode_json($json_string)
    {
        if (is_null($json_string) || $json_string === '') {
            return [];
        }
