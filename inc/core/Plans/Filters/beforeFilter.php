...
if (uri('segment', 1) == "plans") {
    ...
    if (!empty($configs)) {
        ...
        foreach ($configs as $config) {
            if (isset($config['menu']) && (!isset($config['show_plan']) || $config['show_plan']) && (!isset($config['role']) || $config['role'] == 0) && (($config['hideperm'] ?? false) == false) || (isset($config['show_plan']) && $config['show_plan'])) {
