<?php
    $item_permissions = [];
    if (!empty($item) && !empty($item->permissions)) {
        $decoded_permissions = json_decode($item->permissions, true);
        if (is_array($decoded_permissions)) {
            $item_permissions = $decoded_permissions;
        }
    }
?>
                ...
                <div class="tab-pane fade active show" id="plan_info" role="tabpanel">
                    ...
                    <div class="card border">
                        <div class="card-body">
                            ...
                            <!-- Cloud API Settings -->
                            <div class="border border-dashed border-success rounded p-4 mt-4">
                                <h6 class="text-success mb-3"><i class="fas fa-cloud me-2"></i><?php _e('WhatsApp Cloud API Settings')?></h6>
                                
                                <div class="mb-3">
                                    <label class="form-label"><?php _e('Enable Cloud API')?></label>
                                    <div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="cloud_api_enabled" <?php _e( get_data($item, "cloud_api_enabled", "radio", 1) )?> id="cloud_api_enable" value="1">
                                            <label class="form-check-label" for="cloud_api_enable"><?php _e('Enable')?></label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="cloud_api_enabled" <?php _ec( (get_data($item, "cloud_api_enabled") == 0 || get_data($item, "cloud_api_enabled") == "")?"checked='true'":"" ) ?> id="cloud_api_disable" value="0">
                                            <label class="form-check-label" for="cloud_api_disable"><?php _e('Disable')?></label>
                                        </div>
                                    </div>
                                </div>
                                
                                <div class="mb-3">
                                    <label for="cloud_api_accounts" class="form-label"><?php _e('Number of Cloud API connections')?></label>
                                    <input type="number" class="form-control form-control-solid" id="cloud_api_accounts" name="cloud_api_accounts" value="<?php _e( get_data($item, "cloud_api_accounts", "", "0") )?>">
                                    <span class="fs-10 text-primary"><?php _e("Set -1 for unlimited, 0 to use the same limit as regular accounts")?></span>
                                </div>
                           
                                <div class="mb-3">
                                    <label class="form-label"><?php _e('Enable Meta 1-click connection')?></label>
                                    <div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="cloud_api_embedded_signup" <?php _ec((!array_key_exists("cloud_api_embedded_signup", $item_permissions) || get_data($item_permissions, "cloud_api_embedded_signup") == 1) ? "checked='true'" : "") ?> id="cloud_api_embedded_signup_enable" value="1">
                                            <label class="form-check-label" for="cloud_api_embedded_signup_enable"><?php _e('Enable')?></label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="cloud_api_embedded_signup" <?php _ec((array_key_exists("cloud_api_embedded_signup", $item_permissions) && (int) get_data($item_permissions, "cloud_api_embedded_signup") === 0) ? "checked='true'" : "") ?> id="cloud_api_embedded_signup_disable" value="0">
                                            <label class="form-check-label" for="cloud_api_embedded_signup_disable"><?php _e('Disable')?></label>
                                        </div>
                                    </div>
                                    <span class="fs-10 text-primary"><?php _e("When disabled, the Cloud API section stays available but the automatic Meta connection button is hidden.")?></span>
                                </div>
                                ...
                                <?php if (!empty($rows)): ?>
                                    ...
                                    <div class="tab-pane fade <?php _ec( ($count == 0)?"active show":"" ) ?>" id="plan_<?php _ec($rows[0]['id'])?>" role="tabpanel">
                                        ...
                                        <div class="border border-dashed border-gray-300 rounded p-20 mb-4">
                                            <h5 class="fw-bold m-b-30"><i class="<?php _ec( $rows[0]['icon'] )?>" style="color: <?php _ec( $rows[0]['color'] )?>;"></i> <?php _e( $rows[0]['name'] )?></h5>
                                            <div class="mb-3">
                                        ...
                                        <div class="border border-dashed border-gray-300 rounded p-20 mb-4">
                                            <h5 class="fw-bold m-b-30"><i class="<?php _ec( $icon )?>" style="color: <?php _ec( $color )?>;"></i> <?php _e( $name )?></h5>
                                            <div class="mb-3">
                                                <label for="website_description" class="form-label"> 
                                                    <div class="form-check form-check-inline">
                                                        <input 
