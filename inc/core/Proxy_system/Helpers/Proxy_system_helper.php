...
if( ! function_exists('validate_proxy') ){
    function validate_proxy($proxy = ""){
        $ip = "";
        // Normaliza removendo esquema (http://, https://, socks5://, etc.)
        $proxy = trim($proxy);
        $proxy = preg_replace('#^[a-z]+://#i', '', $proxy);
...
if( !function_exists('proxy_probe_base_url') ){
    function proxy_probe_base_url(){
        $base = trim(get_option('whatsapp_server_url', ''));
        if($base === ''){
        ...
        return rtrim($base, '/') . '/probe_ip_open';
    }
}

if (!function_exists('check_rate_limit')) {
    /**
     * Verifica se o usuário/time excedeu o limite de 10 operações por minuto.
     * Usa session ou cache simples para controle.
     * Retorna true se excedeu o limite, false caso contrário.
     */
    function check_rate_limit($key = null, $limit = 10, $window = 60) {
        if (!$key) {
            $key = 'proxy_rate_' . (get_user('id') ?? 'anon');
        }
        if (!isset($_SESSION)) {
            session_start();
        }
        ...
        $_SESSION[$key] = array_filter($_SESSION[$key], function($ts) use ($window) {
