...
<div class="card b-r-6 h-100 post-schedule wrap-caption">
	<div class="card-header">
		<?php if ( !empty($account) ): ?>
			<h3 class="card-title"><?php _e( sprintf("Autor Call Responder for %s", $account->name) )?></h3>
		<?php else: ?>
	...
	<div class="card-body position-relative">
		...
		<div class="mb-4">
			<label class="form-label"><?php _e("Trigger On")?></label>
			<div>
                    <div class="form-check form-check-inline">
                        ...
                        <label class="form-check-label" for="send_to_all"><?php _e('All Events')?></label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="send_to" <?php _e(get_data($result, "send_to")==2?"checked='true'":"" )?> id="send_to_accepts" value="2">
                        <label class="form-check-label" for="send_to_accepts"><?php _e('Accept Call')?></label>
                    </div>

                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="send_to" <?php _e(get_data($result, "send_to")==3?"checked='true'":"" )?> id="send_to_reject" value="3">
                        <label class="form-check-label" for="send_to_reject"><?php _e('Reject Call')?></label>
                    </div>
                </div>
		</div>

		<div class="mb-4">
			<div class="form-check form-switch form-check-custom form-check-solid form-check-primary d-flex justify-content-between">
				<label class="form-check-label text-gray-700 pe-3" for="auto_reject"><?php _e("Auto reject incoming call")?></label>
				<input class="form-check-input" type="checkbox" role="switch" id="auto_reject" name="auto_reject" value="1" <?php _ec( (int)get_data($result, "auto_reject") === 1 ? "checked='true'" : "" )?>>
			</div>
			<div class="form-text text-gray-500"><?php _e("Available only for Baileys accounts. When enabled, incoming calls are declined before the automatic reply is sent.")?></div>
		</div>


		<ul style="display :none;" class="nav nav-pills mb-3 bg-white rounded fs-14 nx-scroll overflow-x-auto d-flex text-over b-r-6 border" id="pills-tab">
	 	...
	 	<div class="tab-content" id="pills-tabContent">
			...
			<label class="form-label"><?php _e("Response")?></label>
