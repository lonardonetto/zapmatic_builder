<?php
namespace Core\Whatsapp_callresponder\Controllers;

class Whatsapp_callresponder extends \CodeIgniter\Controller
{
    public function __construct(){
        $this->config = parse_config( include realpath( __DIR__."/../Config.php" ) );
        $this->model = new \Core\Whatsapp_callresponder\Models\Whatsapp_callresponderModel();
    }

    protected function getNextCallResponderId()
    {
        $last_item = db_get("id", TB_WHATSAPP_CALLRESPONDER, false, "id", "DESC");

        if (empty($last_item) || !isset($last_item->id)) {
            return 1;
        }
    ...
    public function index( $page = false ) {
        ...
        $data['content'] = view('Core\Whatsapp_callresponder\Views\content', $data_content );
    ...
    public function info() {
        ...
        $ids = post("account");
        $account = db_get("*", TB_ACCOUNTS, ["social_network" => "whatsapp", "category" => "profile", "login_type" => 2, "status" => 1, "ids" => $ids, "team_id" => $team_id]);

        if(!empty($account) || $ids == "all"){
            $result = false;
            if( !empty($account) ){
                $result = db_get("*", TB_WHATSAPP_CALLRESPONDER, [ "instance_id" => $account->token, "team_id" => $team_id ]);
            }
        ...
        return view('Core\Whatsapp_callresponder\Views\info', $data);
    }
    ...
    public function save(){
        ...
        $auto_reject = (int)post('auto_reject');
        $type = (int)post("type");
        $template = 0;
        ...
        if($instance_id != ""){
            $account = db_get("*", TB_ACCOUNTS, ["social_network" => "whatsapp", "category" => "profile", "login_type" => 2, "status" => 1, "token" => $instance_id, "team_id" => $team_id]);
        ...
        if(!empty($account)){
            $item = db_get("*", TB_WHATSAPP_CALLRESPONDER, ["ids" => $account->ids, "team_id" => $team_id]);

            if(!$item ){
                if ($next_insert_id === null) {
                    $next_insert_id = $this->getNextCallResponderId();
                }

                db_insert(TB_WHATSAPP_CALLRESPONDER , [
                    "id" => $next_insert_id,
                    "team_id" => $team_id,
                    "ids" => $account->ids,
                    "type" => $type,
            ...
            $accounts = db_fetch("*", TB_ACCOUNTS, [ "social_network" => "whatsapp", "category" => "profile", "login_type" => 2, "team_id" => $team_id, "status" => 1]);
