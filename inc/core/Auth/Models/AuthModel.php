...
class AuthModel extends Model
{
    ...
    public function login(){
		...
		$sql = 'SELECT * FROM '. TB_USERS . " WHERE ( email = ?  OR username = ?) AND password = ?";
		$query = $db->query($sql, array($username, $username, password($password)));
		$user = $query->getRow(); 
		$query->freeResult();
...
public function sendotp(){
    ...
    public function signup(){
		...
		$email = post("email");
		$code = post("code");
		$whatsapp = post("number");
		$password = post("password");
		$confirm_password = post("confirm_password");
		$agree_terms = post("agree_terms");
		$timezone = post("timezone");
		$opt_verified = post("opt_verified");
		...
		$phone = $code . $whatsapp;;
        $remover = array("(", ")", "-", " ", "+", ".", "/", "*", "_", "#");
        $phone = str_replace($remover, "", $phone);
        if (substr($phone, 0, 2) == '55') {
        $ddd = substr($phone, 2, 2);
        if ($ddd >= 31 && strlen($phone) >= 13) {
        ...
        $check_whatsapp = db_get("id", TB_USERS, ["whatsapp" => $phone]);
        validate("not_empty", __("WhatsApp já é cadastrado."), $check_whatsapp);
    ...
    public function recovery_password()
{
    ...
    validate("other", __("New password and confirm password do not match."), $new_password, $confirm_new_password);
    validate("null", __("Recovery key"), $recovery_key);

    $user = db_get("id,fullname,status,ids,whatsapp", TB_USERS, ["recovery_key" => $recovery_key]);
    ...
    if (empty($error)) {
        ...
        if ($success) {
            ...
            $phone = $user->whatsapp;
            $success_message =  get_option('recovery_password_success');
            $success_message = str_replace("{fullname}", $user->fullname, $success_message);
            $message = strip_tags(html_entity_decode(html_entity_decode($success_message)));
        ...
        ms([
            ...
            $phone = $user->whatsapp;
            $error_message =  get_option('recovery_password_error');
            $error_message = str_replace("{fullname}", $user->fullname, $error_message);
            $message = strip_tags(html_entity_decode(html_entity_decode($error_message)));
            send_whats($phone, $message);
    }
  }
