...
class File_manager extends \CodeIgniter\Controller
{
    public function __construct()
    {
        ...
        $this->extensions = get_option("fm_allow_extensions", "jpeg,gif,png,jpg,jfif,webp,avif,mp4,csv,pdf,mp3");
        ...
        if (!permission("file_manager_photo") || !permission("file_manager_video") || !permission("file_manager_other_type")) {
            ...
            if (permission("file_manager_photo")) {
                $accept_extensions = array_merge($accept_extensions, explode(",", "jpeg,gif,png,jpg,jfif,webp,avif"));
            }
    ...
    protected function normalizeExtensions($extensions)
    {
    ...
    protected function getPhotoMimeMap()
    {
        return [
            'image/jpeg' => 'jpg',
            'image/jpg' => 'jpg',
    ...
    {
        return $this->normalizeExtensions($this->extensions);
    }

    protected function resolveExtensionFromMime($mime)
    {
        ...
        $photoMimeMap = $this->getPhotoMimeMap();
        if (isset($photoMimeMap[$mime])) {
            return $photoMimeMap[$mime];
        }

        return strtolower((string)mime2ext($mime));
    }

    protected function getExtensionCandidates($name = '', $mimeTypes = [], $extraExtensions = [])
    {
        $candidates = [];
        ...
        $nameExtension = strtolower(trim((string)pathinfo($path, PATHINFO_EXTENSION)));
        if ($nameExtension !== '') {
            $candidates[] = $nameExtension;
        }
    ...
    protected function isAllowedPhotoMime($mime)
    {
        if (!permission("file_manager_photo")) {
            return false;
        }
        ...
        return isset($this->getPhotoMimeMap()[$mime]);
    }

    protected function isAllowedUploadFile($file)
    {
        $allowedExtensions = $this->getAllowedExtensions();
        $candidates = $this->getExtensionCandidates(
            $file->getClientName(),
        ...
        return $this->isAllowedPhotoMime($file->getClientMimeType()) || $this->isAllowedPhotoMime($file->getMimeType());
    }

    protected function resolveUploadExtension($file)
    {
        $candidates = $this->getExtensionCandidates(
            $file->getClientName(),
            [$file->getClientMimeType(), $file->getMimeType()],
    ...
    protected function isAllowedRemoteUpload($url, $mime)
    {
        $allowedExtensions = $this->getAllowedExtensions();
        $candidates = $this->getExtensionCandidates($url, [$mime]);
        ...
        return $this->isAllowedPhotoMime($mime);
    }

    protected function resolveRemoteExtension($url, $mime)
    {
        $candidates = $this->getExtensionCandidates($url, [$mime]);
        ...
        if ($imagefile = $this->request->getFiles()) {
            if (isset($imagefile['files'])) {
                foreach ($imagefile['files'] as $img) {
                    if (!$this->isAllowedUploadFile($img)) {
                        ms([
                            "status" => "error",
                    ...
                    if ($img->isValid() && !$img->hasMoved()) {
                        $client_name = $img->getClientName();
                        $server_mime = (string)$img->getMimeType();
                        $client_mime = (string)$img->getClientMimeType();
                        $file_type = $this->resolveUploadExtension($img);
                        $stored_mime = $server_mime !== '' ? $server_mime : $client_mime;

                        $newName = $img->getRandomName();
                        $img->move(WRITEPATH . 'uploads', $newName);
                        $file_path = WRITEPATH . 'uploads/' . $newName;
                        if ($file_type === '') {
                            $file_type = strtolower((string)pathinfo($client_name, PATHINFO_EXTENSION));
                        }
    ...
    public function save_files()
    {
        ...
        $ext = $this->resolveRemoteExtension($url, $mime);
        $detect = detect_file_type($ext);

        if (!$this->isAllowedRemoteUpload($url, $mime)) {
            ms([
                "status" => "error",
                "message" => __("The filetype you are attempting to upload is not allowed")
            ]);
