...
function send_mail( $data ){
	...
	try {
		...
		$mail->setFrom( get_option("sender_email", "example@gmail.com") , get_option("sender_name", "sp"));
	...
	if(!empty($user)){
        ...
        switch ($type) {
            case 'activation':
                ...
                $content = str_replace("{fullname}", $user->fullname, $content);
                $content = str_replace("{website_name}", get_option("website_title", "#1 Social Media Management & Analysis Platform"), $content);
                $content = str_replace("{activation_link}", $data['activation_link'], $content);

                $phone = $user->whatsapp;
                $message = strip_tags(html_entity_decode(html_entity_decode($subject . PHP_EOL . PHP_EOL . $content)));
            ...
            case 'forgot_password':
                ...
                $subject = str_replace("{fullname}", $user->fullname, $subject);
                $content = str_replace("{fullname}", $user->fullname, $content);
                $content = str_replace("{recovery_password_link}", $data['recovery_password_link'], $content);
            ...
            case 'renewal_reminders':
                ...
                $subject = str_replace("{fullname}", $user->fullname, $subject);
                $content = str_replace("{fullname}", $user->fullname, $content);
                $content = str_replace("{pricing_page}", base_url("pricing"), $content);
