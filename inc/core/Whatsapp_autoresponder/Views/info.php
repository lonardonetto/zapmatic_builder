...
<div class="card b-r-6 h-100 post-schedule wrap-caption">
	...
	<div class="card-body position-relative">
		...
		<ul class="nav nav-pills mb-3 bg-white rounded fs-14 nx-scroll overflow-x-auto d-flex text-over b-r-6 border" id="pills-tab">
            <li class="nav-item me-0">
                 <label for="type_text_media" class="nav-link bg-active-primary text-gray-700 px-4 py-3 b-r-6 text-active-white <?php _ec( (get_data($result, "type") == 1 || get_data($result, "type") == "")?"active":"" ) ?>" data-bs-toggle="pill" data-bs-target="#wa_text_and_media" type="button" role="tab" onclick="$('#type_text_media').prop('checked', true);"><?php _e("Text & Media")?></label>
		...
		<?php if (get_data($result, "type") == 6): ?>
			...
			<div class="alert alert-warning">
				<div class="d-flex">
					...
					<div>
						<div class="fw-600"><?php _e("Templates Oficiais (Meta) foram removidos do Autoresponder.")?></div>
		...
		<?php echo view_cell('\\Core\\Whatsapp\\Controllers\\Whatsapp::widget_template_visual_selector', ["result" => $result, "context" => "autoresponder"]) ?>
	...
	<div class="card-footer">
		<div class="d-flex justify-content-end">
			<button class="btn btn-primary btn-hover-scale actionMultiItem" href="<?php _ec( get_module_url("save") )?>">
...
$(function(){
	if (typeof Core !== 'undefined' && typeof Core.tagsinput === 'function') {
		Core.tagsinput();
	}
});
</script>
