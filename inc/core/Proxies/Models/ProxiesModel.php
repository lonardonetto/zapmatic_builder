...
class ProxiesModel extends Model
{
    ...
    public function get_account_use_proxies($path = ""){
        ...
        // Adiciona as contas do WhatsApp à lista
        $team_id = get_team("id");
        $whatsapp_accounts = db_fetch("id", TB_ACCOUNTS, ["social_network" => "whatsapp", "team_id" => $team_id]);
        if (!empty($whatsapp_accounts)) {
            foreach ($whatsapp_accounts as $wa_account) {
                $accounts[] = $wa_account->id;
            }
        }

        return array_unique($accounts); // Usa array_unique para evitar duplicatas
    }
}
