...
class Proxies extends \CodeIgniter\Controller
{
    ...
    // Proxy server-side para consultar localização (chama o Node interno e retorna JSON)
    public function probe_location_ajax(){
        try{
            $id = $this->request->getGet('id') ?? post('id');
            if(!$id){
                return $this->response->setJSON(['status'=>'error','message'=>'ID não informado']);
            }
            // O grid usa "ids" (string). Busque também por id numérico como fallback
            $teamId = get_team('id');
            $account = db_get("*", TB_ACCOUNTS, ["ids" => $id, "team_id" => $teamId]);
            if(!$account){
                $account = db_get("*", TB_ACCOUNTS, ["id" => (int)$id, "team_id" => $teamId]);
            }
            if(!$account || empty($account->token)){
                return $this->response->setJSON(['status'=>'error','message'=>'Conta não encontrada']);
            }
            helper('Core\Proxy_system\Helpers\Proxy_system_helper');
            $probeBase = proxy_probe_base_url();
            $url = $probeBase . '?instance_id=' . urlencode($account->token) . '&force=1';
            $ch = curl_init($url);
            ...
            if($resp === false || $http < 200 || $http >= 300){
                return $this->response->setJSON(['status'=>'error','message'=>'Falha ao consultar','http'=>$http,'error'=>$err,'url'=>$url]);
            }
