		...
		<tr class="item">
		    ...
		    <td class="d-flex align-items-center border-bottom py-3">
				...
				<div class="d-flex flex-column">
					<span class="fw-bold text-gray-800"><?php _ec( $value->account_name )?></span>
		        	<span class="text-gray-400"><?php _ec( ucfirst( str_replace("_", " ", $value->social_network) ) )?></span>
				</div>
		    </td>
		    <td class="border-bottom py-3">
		    	<?php if($value->proxy): ?>
		    		<span class="badge badge-light-success"><?php _ec( $value->proxy )?></span>
		    	<?php else: ?>
					<span class="badge badge-light-danger"><?php _e("No proxy")?></span>
		    	<?php endif; ?>
		    </td>
		    <td class="border-bottom py-3 pe-4">
		    	<?php if ($value->location): ?>
		    		<i class="flag-icon flag-icon-<?php _ec( strtolower($value->location) )?>"></i> <?php _ec( list_countries($value->location) )?>
		    	<?php endif ?>
			</td>
		</tr>
