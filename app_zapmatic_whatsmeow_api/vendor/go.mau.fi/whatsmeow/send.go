...
// addInteractiveBizNodes adds the XML biz/interactive/native_flow/quality_control nodes
// to content, exactly as Baileys does in messages-send.js getButtonArgs().
// This is the XML metadata that WhatsApp requires to render interactive messages.
func addInteractiveBizNodes(content *[]waBinary.Node, buttonType string, buttonAttrs waBinary.Attrs) {
	bizAttrs := waBinary.Attrs{
		"actual_actors":   "2",
		"host_storage":    "2",
		"privacy_mode_ts": fmt.Sprintf("%d", time.Now().UnixNano()/1000000),
	}
	interactiveNode := waBinary.Node{
		Tag:   buttonType,
		Attrs: buttonAttrs,
...
func (cli *Client) getMessageContent(
	...
	if buttonType := getButtonTypeFromMessage(message); buttonType != "" {
		if buttonType == "interactive" {
			addInteractiveBizNodes(&content, buttonType, getButtonAttributes(message))
		} else {
			content = append(content, waBinary.Node{
				Tag: "biz",
				Content: []waBinary.Node{{
