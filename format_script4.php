...
// Extract section: "Instance Api" block and everything after it
...
// Substitui os títulos de seção (<h5...>)
$tail = str_replace('<h5 class="border-bottom m-b-30 p-b-20 text-dark text-uppercase">', '</div></div></div><h3 class="swagger-section-title">', $tail);
$tail = str_replace('</h5>', '</h3>', $tail);

// Remove the structural divs that wrapped sections originally
$tail = str_replace('<div class="row p-t-25 p-b-25">', '', $tail);
$tail = str_replace('<div class="col-12">', '', $tail);
...
function processSimpleBlocks($text) {
    // Regex para pegar <h6>...</h6> <div class="alert...">...</div> <div class="text">...</div> <div class="text-uppercase...">...</div> <table...>...</table>
...
// Fix param names styling
$tail = preg_replace('/<tr>\s*<td>(.*?)<\/td>/i', '<tr><td class="param-name">$1</td>', $tail);

// Clean up residual closing divs that we removed earlier
$tail = str_replace('</div></div></div><h3', '<h3', $tail);
$tail = preg_replace('/<\/div>\s*<\/div>\s*<\/div>\s*$/', '</div></div></div>', $tail);
